...
``trustzone_hello`` demonstrates the Armv8-M **TrustZone** split in no-OS's
CAPI (Common API) layer: the device boots **Secure**, configures the security
...
#. **Nested Non-Secure build** -> ``nonsecure.bin``. A second CMake tree
   ...
   ``PROJECT_DEFCONFIG=trustzone_hello/nonsecure.conf``, linking
...
automatically - no extra flag is needed. Building the ``trustzone_hello``
...
* ``trustzone_hello.elf`` - the combined Secure + Non-Secure image.
* ``trustzone_hello.hex`` - Intel HEX, the flashable artifact.
...
.. code-block:: bash
	...
	arm-none-eabi-objdump -h build/build/trustzone_hello.elf
	arm-none-eabi-nm      build/build/trustzone_hello.elf | grep IncrementCount_S
