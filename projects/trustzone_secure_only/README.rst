...
``trustzone_secure_only`` builds **only the Secure world** of a MAX32657
...
``trustzone_nonsecure_only`` (the **consumer**), links against this project's
...
This contrasts with ``max32657_tz_hello`` / ``trustzone_selftest``, which link
...
Building the ``trustzone_secure_only`` target produces, in ``build/build/``:
...
.. list-table::
	...
	* - ``trustzone_secure_only_implib.o``
	...
	* - ``trustzone_secure_only_contract.cmake``
...
.. code-block:: bash
	...
	arm-none-eabi-objdump -h build-sec/build/trustzone_secure_only.elf
	arm-none-eabi-nm      build-sec/build/trustzone_secure_only_implib.o
...
::
	...
	projects/trustzone_secure_only/
