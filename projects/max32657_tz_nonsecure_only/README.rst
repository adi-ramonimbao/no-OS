...
``max32657_tz_nonsecure_only`` builds **only the Non-Secure world** of a
...
``max32657_tz_secure_only`` (the **producer**), emits the Secure image, the CMSE
...
.. code-block:: bash
	...
	cmake --preset max32657evkit -B build-ns \
	    -DPROJECT_DEFCONFIG=max32657_tz_nonsecure_only/nonsecure.conf \
	    -DSECURE_CONTRACT=$PWD/build-sec/build/max32657_tz_secure_only_contract.cmake
	cmake --build build-ns --target max32657_tz_nonsecure_only
...
Provision the Secure world once (flash ``max32657_tz_secure_only``), then
...
.. code-block:: bash
	...
	cmake --preset max32657evkit -B build-ns \
	    -DPROJECT_DEFCONFIG=max32657_tz_nonsecure_only/nonsecure.conf \
	    -DSECURE_IMPLIB=$PWD/build-sec/build/max32657_tz_secure_only_implib.o
	# iterate:
	cmake --build build-ns --target max32657_tz_nonsecure_only
...
* ``max32657_tz_nonsecure_only_nonsecure.hex`` - the Non-Secure image on its own
  (at ``0x01080000``). This is what an NS-only flash programs.
* ``max32657_tz_nonsecure_only.hex`` - with a Secure HEX supplied, the
...
.. code-block:: bash
	...
	arm-none-eabi-objdump -h build-ns/build/max32657_tz_nonsecure_only.elf
	# combined HEX should carry three upper-address regions: 0108 (NS flash),
	# 1100 (Secure .text), 1107 (NSC veneers)
	grep -iE '^:02000004' build-ns/build/max32657_tz_nonsecure_only.hex | sort -u
...
* ``projects/max32657_tz_secure_only`` - the producer that emits the Secure HEX,
