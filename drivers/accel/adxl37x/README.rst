...
|                            |                     |                     | required            |
+----------------------------+---------------------+---------------------+---------------------+
| ADXL37x_WAKE_UP            | Standby mode for    | 1                   | Ideal for simple    |
...
The ADXL37x Impact Detection features allow the device to capture impact events
...
The below example shows an example configuration for using the ADXL37x in
...
.. code-block:: C
	 ...
	 */
   	...
   	void adxl_configure_impact(struct adxl37x_dev *dev)
   	{
   		int err;
   		...
   		adxl37x_set_op_mode(dev, ADXL37x_STANDBY);

   		/* Generic ADXL Part parameters */
   		adxl37x_set_autosleep(dev, true);
   		adxl37x_set_bandwidth(dev, ADXL37x_BW_2560HZ);
   		adxl37x_set_odr(dev, ADXL37x_ODR_5120HZ);
   		adxl37x_set_wakeup_rate(dev, ADXL37x_WUR_65ms);
   		adxl37x_set_act_proc_mode(dev, ADXL37x_LOOPED);

   		/* Set Instant On threshold (Low threshold 10-15 G) */
   		adxl37x_set_instant_on_th(dev, ADXL37x_INSTANT_ON_HIGH_TH);
   		...
   		dev->fifo_config.fifo_mode = ADXL37x_FIFO_STREAMED;
   		dev->fifo_config.fifo_format = ADXL37x_XYZ_PEAK_FIFO;
   		dev->fifo_config.fifo_samples = ACCEL_MAX_SAMPLES;
   		adxl37x_configure_fifo(dev,
   		...
   		adxl37x_set_activity_threshold(dev, ADXL37x_ACTIVITY, ACT_THRESHOLD,
   					       true, true);
   		adxl37x_set_activity_threshold(dev, ADXL37x_INACTIVITY, INACT_THRESHOLD,
   					       true, true);

   		/* Set activity/inactivity time settings */
   		adxl37x_set_activity_time(dev, ACT_TIME);
   		adxl37x_set_inactivity_time(dev, INACT_TIME);

   		/* Set instant-on interrupts and activity interrupts */
   		adxl37x_interrupt_config(dev, int1_config, int2_config);

   		/* Set filter settle time */
   		adxl37x_set_filter_settle(dev, ADXL37x_FILTER_SETTLE_4divODR);

   		/* Disable LPF */
   		adxl37x_write_mask(dev,
   				   ADXL37x_POWER_CTL,
   				   ADXL37x_POWER_CTL_LPF_DIS_MSK,
   				   ADXL37x_POWER_CTL_LPF_DIS_MODE(0b1));

   		/* Disable HPF */
   		adxl37x_write_mask(dev,
   				   ADXL37x_POWER_CTL,
   				   ADXL37x_POWER_CTL_HPF_DIS_MSK,
   				   ADXL37x_POWER_CTL_HPF_DIS_MODE(0b1));

   		/*
   		 * Enable full-BW Measurement to set the reference
   		 * if using internally referenced act/inact thresholds
   		 */
   		adxl37x_set_op_mode(dev, ADXL37x_FULL_BW_MEASUREMENT);
   		no_os_mdelay(10);
   		adxl37x_set_op_mode(dev, ADXL37x_STANDBY);

   		/* Clear fifo completely before enabling Instant On mode */
   		err = adxl37x_get_status(dev, &status1, &status2, &fifo_entries);
   		if (err) {
   			printf("ERR Getting Status %d\n", err);
   			while(1);
   		}
   		adxl37x_get_fifo_xyz_data_workaround(dev, accel_data, fifo_entries);

   		/* Clear all status flags */
   		adxl37x_get_status(dev, &status1, &status2, &fifo_entries);

   		/* Set operation mode to Instant-On */
   		adxl37x_set_op_mode(dev, ADXL37x_INSTANT_ON);
   	}
...
The "Triggered Mode" feature of the ADXL37x allows for full event detection
...
.. code-block:: C
	...
	#define EVENTS_TRAILING_SAMPLES 270
   	...
   	void adxl_configure_triggered_mode(struct adxl37x_dev *dev)
   	{
   		int err;
   		struct adxl37x_irq_config int1_config = {
   			.activity = false, .inactivity = false,
   		...
   		/* Generic ADXL Part parameters */
   		adxl37x_set_bandwidth(dev, ADXL37x_BW_2560HZ);
   		adxl37x_set_odr(dev, ADXL37x_ODR_2560HZ);
   		adxl37x_set_act_proc_mode(dev, ADXL37x_LOOPED);
		 ...
		 *
   		...
   		/* Put fifo in Peak Detect and streamed Mode */
   		dev->fifo_config.fifo_mode = ADXL37x_FIFO_TRIGGERED;
   		dev->fifo_config.fifo_format = ADXL37x_XYZ_FIFO;
