...
typedef void (*capi_rtc_event_callback_t)(struct capi_rtc_handle *handle, uint32_t event,
					  void *ctx);

/**
 * @brief Initialize the RTC peripheral
 * @param[in] handle Pointer to handle pointer (can be NULL for driver allocation)
 * @param[in] config RTC configuration
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_init(struct capi_rtc_handle **handle, const struct capi_rtc_config *config);

/**
 * @brief Deinitialize the RTC peripheral
 * @param[in] handle The RTC handle
 * @return 0 on success, negative error code otherwise
 */
 ...
 * @param[in] handle The RTC handle
 * @param[out] time Pointer to store current time
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_get_time(struct capi_rtc_handle *handle, struct capi_rtc_time *time);

/**
 * @brief Set the current time
 * @param[in] handle The RTC handle
 * @param[in] time Pointer to new time value
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_set_time(struct capi_rtc_handle *handle, const struct capi_rtc_time *time);

/**
 * @brief Get the current datetime
 * @param[in] handle The RTC handle
 * @param[out] datetime Pointer to store current datetime
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_get_datetime(struct capi_rtc_handle *handle, struct capi_rtc_datetime *datetime);

/**
 * @brief Set the current datetime
 * @param[in] handle The RTC handle
 * @param[in] datetime Pointer to new datetime value
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_set_datetime(struct capi_rtc_handle *handle, const struct capi_rtc_datetime *datetime);

/**
 * @brief Set alarm
 * @param[in] handle The RTC handle
 * @param[in] type Alarm type
 * @param[in] alarm_value Pointer to alarm value (uint32_t, capi_rtc_time, or
 * 			  capi_rtc_datetime depending on type)
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_set_alarm(struct capi_rtc_handle *handle, enum capi_rtc_alarm_type type,
 ...
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_disable_alarm(struct capi_rtc_handle *handle, enum capi_rtc_alarm_type);

/**
 * @brief Enable square wave output
 * @param[in] handle The RTC handle
 * @param[in] freq The square wave frequency
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_sqwave_enable(struct capi_rtc_handle *handle, enum capi_rtc_sqwave_freq freq);
 ...
 * @param[in] event_ctx Context pointer for callback
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_register_callback(struct capi_rtc_handle *handle, capi_rtc_event_callback_t callback,
			       void *event_ctx);

/**
 * @brief Enable RTC events
 * @param[in] handle The RTC handle
 * @param[in] event The event to enable
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_enable_event(struct capi_rtc_handle *handle, uint32_t event);

/**
 * @brief Disable RTC events
 * @param[in] handle The RTC handle
 * @param[in] event The event to disable
 * @return 0 on success, negative error code otherwise
 */
int capi_rtc_disable_event(struct capi_rtc_handle *handle, uint32_t event);
...
struct capi_rtc_ops {
	/** See capi_rtc_init() */
	int (*init)(struct capi_rtc_handle **handle, const struct capi_rtc_config *config);
	/** See capi_rtc_deinit() */
	...
	int (*get_time)(struct capi_rtc_handle *handle, struct capi_rtc_time *time);
	/** See capi_rtc_set_time() */
	int (*set_time)(struct capi_rtc_handle *handle, const struct capi_rtc_time *time);
	/** See capi_rtc_get_datetime() */
	int (*get_datetime)(struct capi_rtc_handle *handle, struct capi_rtc_datetime *datetime);
	...
	int (*set_alarm)(struct capi_rtc_handle *handle, enum capi_rtc_alarm_type type,
			 const void *alarm_value);
	/** See capi_rtc_disable_alarm() */
	int (*disable_alarm)(struct capi_rtc_handle *handle, enum capi_rtc_alarm_type type);
	/** See capi_rtc_sqwave_enable() */
	int (*sqwave_enable)(struct capi_rtc_handle *handle, enum capi_rtc_sqwave_freq freq);
	...
	int (*register_callback)(struct capi_rtc_handle *handle, capi_rtc_event_callback_t callback,
				 void *event_ctx);
	/** See capi_rtc_enable_event() */
	int (*enable_event)(struct capi_rtc_handle *handle, uint32_t event);
	/** See capi_rtc_disable_event() */
	int (*disable_event)(struct capi_rtc_handle *handle, uint32_t event);
	/** See capi_rtc_isr() */
