 ...
 */
typedef void (*capi_trng_callback_t)(enum capi_trng_async_event event, void *arg, int event_extra);
 ...
 * @param[in] handle Pointer to handle pointer (can point to NULL for driver allocation)
 * @param[in] config TRNG configuration
 * @return 0 on succes, negative error code otherwise
 */
int capi_trng_init(struct capi_trng_handle **handle, const struct capi_trng_config *config);

/**
 * @brief Deinitialize the TRNG controller
 * @param[in] handle The TRNG handle
 ...
 * @param[in] handle The TRNG handle
 * @param[out] value Pointer to store the generated value
 * @return 0 on success, negative error code otherwise
 */
 ...
 * @return 0 on success, negative error code otherwise
 */
int capi_trng_fill_buffer(struct capi_trng_handle *handle, uint8_t *buffer, uint32_t length);
 ...
 * @return 0 on success, negative error code otherwise
 */
int capi_trng_fill_buffer_async(struct capi_trng_handle *handle, uint8_t *buffer, uint32_t length);
 ...
 * @return 0 on success, negative error code otherwise
 */
int capi_trng_abort_async(struct capi_trng_handle *handle);

/**
 * @brief Check whether the TRNG is busy or not
 * @param[in] handle The TRNG handle
 * @param[out] is_busy True if TRNG in progress, False if not
 * @return 0 on success, negative error code otherwise
 */
int capi_trng_is_busy(struct capi_trng_handle *handle, bool *is_busy);

/**
 * @brief Register callback for async events
 * @param[in] handle The TRNG handle
 * @param[in] callback User callback function
 * @param[in] callback_arg User context pointer passed to callback
 * @return 0 on success, negative error code otherwise
 */
int capi_trng_register_callback(struct capi_trng_handle *handle, capi_trng_callback_t callback,
				void *callback_arg);
...
struct capi_trng_ops {
	/** See capi_trng_init() */
	int (*init)(struct capi_trng_handle **handle, const struct capi_trng_config *config);
	...
	int (*fill_buffer)(struct capi_trng_handle *handle, uint8_t *buffer, uint32_t length);
	/** See capi_trng_fill_buffer_async() */
	int (*fill_buffer_async)(struct capi_trng_handle *handle, uint8_t *buffer, uint32_t length);
	/** See capi_trng_abort_async() */
	int (*abort_async)(struct capi_trng_handle *handle);
	/** See capi_trng_is_busy() */
	int (*is_busy)(struct capi_trng_handle *handle, bool *is_busy);
	/** See capi_trng_register_callback() */
	int (*register_callback)(struct capi_trng_handle *handle, capi_trng_callback_t callback,
				 void *callback_arg);
